...
export const load: PageServerLoad = (async ({ locals, params }) => {
    ...
    const target_listing_id = params.listing_id;
    if (!user) {
    ...
    try {
        ...
        console.log("schedule page", error);
    ...
    try {
        ...
        const listing =  await locals.pb.collection('listings').getOne(target_listing_id)

        const teacher_profile_id =  listing.teacher;

        const sessions = await locals.pb.collection('sessions').getList(1, 50, { filter: `teacher="${teacher_profile_id}" && start_date>="${formatDate(todayAtMidnight)}" && end_date<="${formatDate(oneWeekFromNow)}"` });
        ...
        return { sessions: mapped_sesions , listing: listing};
    } catch (error) {
        console.log(error);
        return { sessions: [] , listing: undefined};
    }
